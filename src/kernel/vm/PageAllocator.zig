...
const FreeArea = struct {
    ...
    pub fn format(value: *FreeArea, writer: *std.Io.Writer) std.Io.Writer.Error!void {
        ...
        while (curr_node) |node| : (curr_node = node.next) {
            try writer.print("0x{x}", .{getPageNodePhys(node)});
...
pub fn alloc(rank: u8) ?usize {
    if (rank >= max_rank) return null;

    lock.lock();
    defer lock.unlock();

    if (free_areas[rank].list.popFirst()) |page_node| {
        togglePageBit(getPageNodeBase(page_node), rank);

        allocated_pages += @as(u32, 1) << @truncate(rank);
        return getPageNodePhys(page_node);
    }

    var temp_rank = rank + 1;
    const entry = blk: {
        while (temp_rank < max_areas) : (temp_rank += 1) {
            if (free_areas[temp_rank].list.popFirst()) |entry| {
                break :blk entry;
            }
        }
        return null;
    ...
    var temp_base: u32 = getPageNodeBase(entry);

    free_areas[temp_rank - 1].list.prepend(entry);
    ...
    while (temp_rank > rank) {
        ...
        const new_node = nodeFromPage(temp_base);
...
pub fn free(base: usize, rank: u8) void {
    ...
    if (getPageBit(page_base, rank) == 0 or rank == max_rank - 1) {
        const entry = nodeFromPage(page_base);
    ...
    while (getPageBit(page_base, temp_rank) != 0 and temp_rank < max_rank - 1) {
        ...
        const entry = nodeFromPage(buddy_base);
    ...
    const new_node = nodeFromPage(page_base);
    free_areas[temp_rank].list.prepend(new_node);
...
fn pushFreeEntry(entry: *const boot.MemMap.Entry) void {
    ...
    while (temp_pages != 0) {
        ...
        const node = nodeFromPage(temp_base);
...
inline fn nodeFromPage(phys_base: u32) *Node {
...
inline fn getPageNodeBase(node: *Node) u32 {
    return @truncate(getPageNodePhys(node) / vm.page_size);
}
...
inline fn setPageBit(base: u32, rank: u8) void {
    free_areas[rank].bitmap.set(base >> @truncate(1 +% rank));
}

inline fn getPageBit(base: u32, rank: u8) u8 {
