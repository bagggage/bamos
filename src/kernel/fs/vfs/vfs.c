...
Status init_vfs() {
    ...
    GptPartitionNode* partition_node = gpt_partition_list_get_first_node();
    ...
    while (partition_node != NULL) {
        if (is_ext2(partition_node->storage_device, partition_node->partition_entry.lba_start)) {
            ...
            StorageDevice* storage_device = partition_node->storage_device;
            BlockGroupDescriptorTable* bgd_table = (BlockGroupDescriptorTable*)storage_device->interface.read(storage_device,
            (partition_node->partition_entry.lba_start * storage_device->lba_size) + 2 * EXT2_SUPERBLOCK_OFFSET, sizeof(BlockGroupDescriptorTable));
