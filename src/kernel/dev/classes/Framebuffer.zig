...
pub const VariableScreenInfo = extern struct {
    ...
    pub fn setFormat(self: *VariableScreenInfo, format: video.Framebuffer.ColorFormat) void {
        ...
        switch (format) {
            ...
            .RBGA => { self.red = @"4rs"; self.green = @"2nd"; self.blue = @"3rd"; self.transp = @"1st"; },

            .BRGA => { self.red = @"3rd"; self.green = @"2nd"; self.blue = @"4rs"; self.transp = @"1st"; },
            .BGRA => { self.red = @"2nd"; self.green = @"3rd"; self.blue = @"4rs"; self.transp = @"1st"; },

            .ARGB => { self.red = @"3rd"; self.green = @"2nd"; self.blue = @"1st"; self.transp = @"4rs"; },
            .ARBG => { self.red = @"3rd"; self.green = @"1st"; self.blue = @"2nd"; self.transp = @"4rs"; },
            .ABRG => { self.red = @"2nd"; self.green = @"1st"; self.blue = @"3rd"; self.transp = @"4rs"; },
            .ABGR => { self.red = @"1st"; self.green = @"2nd"; self.blue = @"3rd"; self.transp = @"4rs"; },
        }
    }
};
...
const file_ops: devfs.DevFile.Operations = .{
    .open = &devOpen,
    .close = &devClose,
    .fops = .{
        .read = &fileRead,
...
var dev_region: devfs.Region = .{ .major = 29 };
...
pub fn setup(
    ...
    self.* = .{
        .id = id[0..@min(id.len, 15)],
        .ops = ops,
        .dev_file = .{
            .name = dev.Name.print("fb{}", .{ dev_num.minor }) catch unreachable,
            .num = dev_num,
            .ops = &file_ops,
            .access = .{
                .gid = 0,
...
pub fn devOpen(dev_file: *devfs.DevFile, file: *vfs.File) vfs.Error!void {
    const fb = fromDevFile(dev_file);
    file.data = .fromPtr(fb);
}

pub fn devClose(_: *devfs.DevFile, _: *vfs.File) void {}

pub fn fileRead(file: *const vfs.File, offset: usize, buffer: []u8) vfs.Error!usize {
    const fb = fromFile(@constCast(file));
    if (fb.ops.read) |read| return read(fb, offset, buffer);

    return error.BadOperation;
}

pub fn fileWrite(file: *vfs.File, offset: usize, buffer: []const u8) vfs.Error!usize {
    const fb = fromFile(@constCast(file));
    if (fb.ops.write) |write| return write(fb, offset, buffer);

    return error.BadOperation;
}

pub fn fileIoctl(file: *vfs.File, cmd: c_uint, arg: usize) vfs.Error!void {
    const fb = fromFile(file);
    const ioctl: IoCtl = @enumFromInt(cmd);

    const arg_data: lib.AnyData = .from(arg);
    switch (ioctl) {
        .alloc => {
            std.log.info("alloc", .{});
        },
        ...
        .get_fscreen_info => {
            const out = arg_data.asPtr(FixedScreenInfo).?;
            out.* = .{
                .smem_len = fb.size,
                .type = .packed_pixels,
                .type_aux = 0,
        ...
        .get_vscreen_info => {
            ...
            fb.ops.get_capabilities(fb, out);
        },
        .put_vscreen_info => {
...
pub fn fileMmapPrepare(file: *const vfs.File, map_unit: *sys.AddressSpace.MapUnit) vfs.Error!void {
    const fb: *Self = fromFile(@constCast(file));
    return fb.ops.mmap(fb, map_unit);
}
...
inline fn fromDevFile(dev_file: *devfs.DevFile) *Self {
