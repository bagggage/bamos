...
fn buildAuxVectors(bin: *exe.Binary, elf_hdr: *const elf.Header, interp_base: ?usize, phdrs_ptr: usize) exe.Error!void {
    bin.args.alignWriter(@alignOf(elf.Auxv)) catch return error.NoMemory;

    bin.args.writer.print("{f}\x00", .{bin.file.dentry.path()}) catch return error.NoMemory;
    const file_name_ptr = bin.args.getCurrentPtr() + @sizeOf(elf.Auxv) * 2;
    ...
    try bin.args.appendAuxv(elf.AT_PLATFORM, @intFromEnum(builtin.target.toElfMachine()));
