...
pub inline fn delay(iters: u32) void {
    return bindings.getInstance().dev.io.delay(iters);
}
...
pub inline fn request(name: [:0]const u8, base: usize, size: usize, io_type: Type) ?usize {
    return bindings.getInstance().dev.io.request(name, base, size, io_type);
}

pub inline fn release(base: usize, io_type: Type) void {
    return bindings.getInstance().dev.io.release(base, io_type);
}

pub inline fn isAvail(base: usize, size: usize, io_type: Type) bool {
    return bindings.getInstance().dev.io.isAvail(base, size, io_type);
}
