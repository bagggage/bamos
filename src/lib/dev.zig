...
pub const Name = extern struct {
    ...
    pub inline fn format(self: *const Name, writer: *std.Io.Writer) std.Io.Writer.Error!void {
...
pub inline fn registerBus(bus: *Bus) void {
    bindings.getInstance().dev.registerBus(bus);
}

pub inline fn registerDevice(comptime bus_name: []const u8, dev: *Device, driver: ?*const Driver) !void {
...
pub inline fn getBus(comptime name: []const u8) !*Bus {
    ...
    const bus = bindings.getInstance().dev.getBusByHash(hash);

    return bus orelse error.UnsupportedBus;
}

pub inline fn getKernelDriver() *Driver {
    return bindings.getInstance().dev.getKernelDriver();
}
